...
Pixel5<- read.csv("C:/Users/yashs/OneDrive/Documents/archive (6)/GooglePixel5_reviews.csv")
#Check dimension of data
dim(Pixel5)#Shows number of rows and columns
class(Pixel5)
#Convert data into a corpus
mycorpus<- VCorpus(VectorSource(Pixel5$userComments)) #Create a volatile corpus
...
writeLines(as.character(mycorpus[[124]]))
...
model1<- LDA(mat,k=7) #select five topics for our analysis
ldout.topics<-as.matrix(topics(model1))
write.csv(ldout.topics,"topic_label.csv",row.names= TRUE)

#Build key words related to each topic
ldaout.terms<-(terms<-terms(model1,10))
write.csv(ldaout.terms,"terms.csv",row.names = FALSE)
...
Pixel5<- read.csv("C:/Users/13345/Dropbox/My PC (DESKTOP-B1D9GNS)/Downloads/archive (6)/GooglePixel5_reviews.csv")
sentiment<- iconv(Pixel5$userComments)
